function [out, a, Error, b, w] = takeStep(i1, i2, a, Y, b, eps, Error, C, K, E2, tol, w, X_train)
    ...
    if (eta < 0)
        ...
        Lobj = -s*L+L-0.5*k11*(gamma-s*L)^2-.5*k22*L^2-s*k12*(gamma-s*L)*L-Y(i1)...
            *(gamma-s*L)*(f(Y,a,b,i1,K)+b-Y(i1)*alph1*k11-Y(i2)*alph2*K(i2,i1))...
            -Y(2)*L*(f(Y,a,b,i2,K)+b-Y(i1)*alph1*k12-Y(i2)*alph2*k22);

        
        Hobj = -s*H+H-0.5*k11*(gamma-s*H)^2-.5*k22*H^2-s*k12*(gamma-s*H)*H-Y(i1)...
            *(gamma-s*H)*(f(Y,a,b,i1,K)+b-Y(i1)*alph1*k11-Y(i2)*alph2*K(i2,i1))...
            -Y(2)*H*(f(Y,a,b,i2,K)+b-Y(i1)*alph1*k12-Y(i2)*alph2*K(i2,i1));
